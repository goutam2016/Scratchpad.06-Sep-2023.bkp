...
object PersonOperations {
    ...
    def maxSalaryPerDept(employees: IndexedSeq[Employee]): Map[String, BigDecimal] = {
        employees.groupBy(_.getDepartment()).mapValues(_.map(_.getYearlyGrossSalary())).mapValues(_.max)
    }

    def empNameWithMaxSalaryPerDept(employees: IndexedSeq[Employee]): Map[String, String] = {
        employees.groupBy(_.getDepartment()).mapValues(_.maxBy(_.getYearlyGrossSalary())).mapValues(emp => String.join(" ", emp.getFirstName(), emp.getLastName()))
    }
    ...
    def employeeCountPerSexPerDept(employees: IndexedSeq[Employee], baseSalary: BigDecimal): Map[String, Map[String, Int]] = {
        ...
        val employeesPerSexPerDept = employees.filter(_.getYearlyGrossSalary() >= baseSalary).groupBy(_.getDepartment()).mapValues(_.groupBy(sexClassifier))
        employeesPerSexPerDept.mapValues(_.mapValues(_.size))
    }
    ...
    def averageSalaryPerDepartment(employees: IndexedSeq[Employee]): Map[String, BigDecimal] = {
        ...
        val summedSalaryPerDept = employees.groupBy(_.getDepartment()).mapValues(_.map(_.getYearlyGrossSalary())).mapValues(salaries => new Tuple2(salaries.size, salaries.reduce(salaryAdder)))
        summedSalaryPerDept.mapValues(salaryCntVsSummedSalary => salaryCntVsSummedSalary._2 / BigDecimal.valueOf(salaryCntVsSummedSalary._1))
    }
}