...
public class RDDTripAnalyzer implements TripAnalyzer {
	...
	public Map<Month, MaxMinFares> getMaxMinFaresPerMonth(TimeBand timeBand, boolean useCaching) {
		Map<Month, BigDecimal> maxFarePerMonth = taxiTrips
				.filter(trip -> trip.getPickupDateTime().toLocalTime().equals(timeBand.getStartTime())
						|| trip.getPickupDateTime().toLocalTime().isAfter(timeBand.getStartTime()))
				.filter(trip -> trip.getPickupDateTime().toLocalTime().isBefore(timeBand.getEndTime()))
				.mapToPair(trip -> new Tuple2<>(trip.getPickupDateTime().getMonth(), trip.getFareAmount()))
				.reduceByKey(BigDecimal::max).collectAsMap();
		maxFarePerMonth.forEach(
				(month, maxFare) -> System.out.printf("Month: %s, maximum fare: %d.\n", month, maxFare.intValue()));

		/*TaxiTrip maxFareTrip = taxiTrips
		.filter(trip -> trip.getPickupDateTime().toLocalTime().equals(timeBand.getStartTime())
				|| trip.getPickupDateTime().toLocalTime().isAfter(timeBand.getStartTime()))
		.filter(trip -> trip.getPickupDateTime().toLocalTime().isBefore(timeBand.getEndTime()))
		.max(new FareAmtComparator());
		System.out.println("maxFareTrip: " + maxFareTrip.getFareAmount() + ", date: " + maxFareTrip.getPickupDateTime().toLocalDate());*/
		/*Map<Month, BigDecimal> maxFaresPerMonth = taxiTrips
				.filter(trip -> trip.getPickupDateTime().toLocalTime().equals(timeBand.getStartTime())
						|| trip.getPickupDateTime().toLocalTime().isAfter(timeBand.getStartTime()))
				.filter(trip -> trip.getPickupDateTime().toLocalTime().isBefore(timeBand.getEndTime()))
				.mapToPair(trip -> new Tuple2<>(trip.getPickupDateTime().getMonth(), trip))
				.aggregateByKey(BigDecimal.ZERO,
						(maxFareAmt, trip) -> maxFareAmt.doubleValue() >= trip.getFareAmount().doubleValue()
								? maxFareAmt
								: trip.getFareAmount(),
						(maxFareAmt1, maxFareAmt2) -> maxFareAmt1.doubleValue() >= maxFareAmt2.doubleValue()
								? maxFareAmt1
								: maxFareAmt2)
				.collectAsMap();
		maxFaresPerMonth
				.forEach((month, maxFare) -> System.out.printf("Month: %s, maximum fare: %d.\n", month, maxFare.intValue()));*/
		
				/*.reduceByKey(
						(trip1, trip2) -> trip1.getFareAmount().compareTo(trip2.getFareAmount()) >= 0 ? trip1 : trip2)
				.mapValues(trip -> createMaxMinFares(timeBand, trip, null)).collectAsMap();*/

		// .mapToPair(trip -> new Tuple2<>(trip.getPickupDateTime().getMonth(), trip.getPassengerCount()))
		return new HashMap<>();
	}
	
	private static class FareAmtComparator implements Comparator<TaxiTrip>, Serializable {

		private static final long serialVersionUID = -7593758330274482955L;

		@Override
		public int compare(TaxiTrip trip1, TaxiTrip trip2) {
			return trip1.getFareAmount().compareTo(trip2.getFareAmount());
		}
		
	}
	
	private MaxMinFares createMaxMinFares(TimeBand timeBand, TaxiTrip tripWithMaxFare, TaxiTrip tripWithMinFare) {
		MaxMinFares maxMinFares = new MaxMinFares(timeBand);

		if (tripWithMaxFare != null) {
			maxMinFares.setMaxFareAmount(tripWithMaxFare.getFareAmount());
		}
		if (tripWithMinFare != null) {
			maxMinFares.setMinFareAmount(tripWithMinFare.getFareAmount());
		}
		return maxMinFares;
	}
